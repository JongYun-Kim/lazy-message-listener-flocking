...
if __name__ == "__main__":
    ...
    # algo_colors = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#000000", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#000000"]
    algo_colors = ["#000000", "red",
                   "#2ca02c", "#1f77b4", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#ff7f0e",
                   "#2ca02c", "#1f77b4", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#ff7f0e"]
    ...
    for episode in range(num_seeds):
        ...
        for algo_index in range(num_algos):
            if algo_str[algo_index].startswith("topology"):
                continue  # plot metrics as well as RL and ACS
            ...
            #
            # Determine line style: dotted for "ACS" or "RL", solid for others
            # ACS: dashed, RL: dotted, else solid
            if algo_str[algo_index] in ["ACS"]:
                linestyle = 'dashed'
            elif algo_str[algo_index] in ["RL"]:
                linestyle = 'dashed'
            else:
                linestyle = '-'

            # Plot with specified line style and width
            # axes[0].plot(times, spatial_entropy[episode, algo_index, :], label=algo_str[algo_index],
            #              linewidth=2.0, linestyle=linestyle)
            # axes[1].plot(times, velocity_entropy[episode, algo_index, :], label=algo_str[algo_index],
            #              linewidth=2.0, linestyle=linestyle)
            temp_str = ['ACS', 'RL', 'Dist_1', 'Dist_2', 'Dist_3', 'Dist_4', 'Dist_5', 'Dist_6', 'Dist_7', 'Dist_8', 'Dist_9',
             'Topo_1', 'Topo_2', 'Topo_3', 'Topo_4', 'Topo_5', 'Topo_6', 'Topo_7', 'Topo_8', 'Topo_9']
            axes[0].plot(times, spatial_entropy[episode, algo_index, :], label=temp_str[algo_index],
                         color=algo_colors[algo_index], linewidth=2.0, linestyle=linestyle)
            axes[1].plot(times, velocity_entropy[episode, algo_index, :], label=temp_str[algo_index],
                         color=algo_colors[algo_index], linewidth=2.0, linestyle=linestyle)
        ...
        axes[1].legend(loc='upper center', bbox_to_anchor=(0.5, -0.17), ncol=6, fontsize=18)
        ...
        plt.savefig(folder_path + "plots/" + f"entropy_metrics_seed_{seeds[episode]}.png", dpi=my_dpi)
        plt.close()
        ...
        for algo_index in range(num_algos):
            if algo_str[algo_index].startswith("metric"):
                continue  # plot topology as well as RL and ACS
            # Plot with line width set to 2.0
            # axes[0].plot(times, spatial_entropy[episode, algo_index, :], label=algo_str[algo_index],
            #              color=algo_colors[algo_index], linewidth=2.0)
            # axes[1].plot(times, velocity_entropy[episode, algo_index, :], label=algo_str[algo_index],
            ...
            # axes[1].plot(times, velocity_entropy[episode, algo_index, :], label=algo_str[algo_index],
            #              linewidth=2.0, linestyle=linestyle)
            temp_str = ['ACS', 'RL', 'Dist_1', 'Dist_2', 'Dist_3', 'Dist_4', 'Dist_5', 'Dist_6', 'Dist_7', 'Dist_8',
                        'Dist_9',
                        'Topo_1', 'Topo_2', 'Topo_3', 'Topo_4', 'Topo_5', 'Topo_6', 'Topo_7', 'Topo_8', 'Topo_9']
            axes[0].plot(times, spatial_entropy[episode, algo_index, :], label=temp_str[algo_index],
                         color=algo_colors[algo_index], linewidth=2.0, linestyle=linestyle)
        ...
        # Save and close the plot
        plt.savefig(folder_path + "plots/" + f"entropy_topology_seed_{seeds[episode]}.png", dpi=my_dpi)
        plt.close()
        ...
        #     #           color=algo_colors[algo_index], linewidth=2.0)
        #     linestyle = 'dashed' if algo_str[algo_index] in ["ACS", "RL"] else '-'
        #     axes.plot(times, -control[episode, algo_index, :] / (u_sat * e_to_u), label=algo_str[algo_index],
        #               linewidth=2.0, color=algo_colors[algo_index], linestyle=linestyle)
        # # Set labels and titles
        ...
        #     #           color=algo_colors[algo_index], linewidth=2.0)
        #     linestyle = 'dashed' if algo_str[algo_index] in ["ACS", "RL"] else '-'
        #     axes.plot(times, average_action, label=algo_str[algo_index], linewidth=2.0, color=algo_colors[algo_index], linestyle=linestyle)
        # # Set labels and titles
        # axes.set_xlabel('Time steps')
        # axes.set_ylabel('Average neighbor selection')
        # axes.set_title('Average Neighbor Selection Over Time')
        # axes.legend()
        # # Set grid with subtle color (with transparency)
        # axes.grid(color='gray', linestyle='--', linewidth=0.5, alpha=0.67)
        # # Ensure x axes start with 0
        ...
        #
        # # Plot 5: Average Action Prob in one plot; each algorithm has its own line; np.sum(actions[episode, algo_index, t, :, :]) / (num_agents)**2
        # # Create a new figure with subplots
        # fig, axes = plt.subplots(1, 1, figsize=(10, 5), constrained_layout=True)
        # for algo_index in range(num_algos):
        #     if algo_str[algo_index].startswith("topology"):
        #         continue
        #     # Plot with line width set to 2.0
        #     average_action_p = np.zeros(max_time_steps)
        #     for t in range(max_time_steps):
        #         average_action_p[t] = (np.sum(action_probs[episode, algo_index, t, :, :])-num_agents) / ((num_agents-1)*num_agents)
        #     # axes.plot(times, average_action_p, label=algo_str[algo_index],
        #     #           color=algo_colors[algo_index], linewidth=2.0)
        #     linestyle = 'dashed' if algo_str[algo_index] in ["ACS", "RL"] else '-'
        #     axes.plot(times, average_action_p, label=algo_str[algo_index], linewidth=2.0, color=algo_colors[algo_index], linestyle=linestyle)
        # # Set labels and titles
        # axes.set_xlabel('Time steps')
        ...
        # axes.set_ylim(bottom=0, top=1.12)
        # # Save and close the plot
        # plt.savefig(folder_path + "plots/" + f"average_neighbor_selection_prob_seed_{seeds[episode]}.png", dpi=my_dpi)
        # plt.close()
        # print(f"Average action probability plot {episode+1}/{num_seeds} saved")
